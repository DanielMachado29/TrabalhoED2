...
void ordenandoComMergeSort(Registro *registros, int N)
{
    ...
    double mediaTempo = 0;
    int M=5;
    cout <<"Ordenando com Merge sort : "<<endl;
    for (int i = 0; i < M; i++)
    {
        high_resolution_clock::time_point inicio = high_resolution_clock::now();
        ord->mergeSort(registros, 0, N);
        high_resolution_clock::time_point fim = high_resolution_clock::now();
        cout << "Exececucao " << i+1 << " : "<< endl;
        cout << "Tempo: " << duration_cast<duration<double>>(fim - inicio).count() << "s.   " ;
        cout << "Comparacoes: " << ord->getQuantidadeComparacoes()<< ".   ";
        cout << "Trocas: " << ord->getQuantidadeTrocas()<< ".   "<<endl;
        ...
        cout<<"ESTOU ORDENANDO PARA "<<N<<" VALORES"<<endl;
    }
    cout<<endl;
...
void menu()
{
    ...
    int tamanhoN[] = {10000, 50000, 100000, 500000, 1000000, TAMANHOMAX};
    Registro *registros = new Registro[tamanhoN[5]];
    Registro *registrosAux = new Registro[tamanhoN[5]]; //tamanho máximo que pode ter
    ...
    while (men != 4)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            //ordenandoComMergeSort(); //registro , tamanho
            //registros->selecionandoRegistrosAleatorios(registros, registrosAux, tamanhoN[i]);
            //registrosAux->

            for(int i = 0 ; i<5 ;i++){
                registros->selecionandoRegistrosAleatorios(registros, registrosAux, tamanhoN[i]);
                ordenandoComMergeSort(registrosAux,tamanhoN[i]);
            }
            break;
        case 4:
            break;
